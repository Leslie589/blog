...
export const login = (req, res) => {
  ...
  db.query(q, [req.body.username], (err, data) => {
    ...
    /* Si las contraseñas no coinciden, devuelve un error 400 (Bad Request)
 con un mensaje indicando que el usuario o la contraseña son incorrectos*/
...
// Establece una cookie llamada "access_token" en la respuesta HTTP
res.cookie("access_token", token, {
  
  // Hace que la cookie **solo sea accesible desde el servidor** (no desde JavaScript en el navegador)
  httpOnly: true,

  // Indica que la cookie **solo se enviará a través de HTTPS**
  secure: true,

  // Permite que la cookie se comparta entre **dominios diferentes** (por ejemplo, frontend y backend en Render)
  sameSite: "none",

// Después de establecer la cookie, se envía la respuesta al frontend
}).status(200).json(other); // Envia un JSON con la info del usuario (sin el password u otros datos sensibles)
